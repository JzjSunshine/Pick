...
def main(config: ConfigParser, local_master: bool, logger=None):
    ...
    train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset) # 分布式数据采集器，将数据分为多个子集，并为每个子集分配一个进程
    ...
    lr_scheduler = config.init_obj('lr_scheduler', torch.optim.lr_scheduler, optimizer) # 实现学习率调度器。在训练过程动态调整学习率的大小，以提高模型性能和稳定性
...
def entry_point(config: ConfigParser):
    ...
    # env_dict = {'MASTER_ADDR': '127.0.0.1', 'MASTER_PORT': '29500', 'RANK': '0', 'WORLD_SIZE': '1'}
...
if __name__ == '__main__':
    ...
    options = [
        ...
        CustomArgs(['--local_rank'], default=1, type=int, target='local_rank',
